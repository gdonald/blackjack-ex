defmodule CardSpec do
  ...
  describe "Card.is_ace?/1" do
    it "returns true" do
      expect Card.is_ace?(ace())
             |> to(be_true())
    end

    it "returns false" do
      card = %Card{value: 1, suit_value: 0}
      expect Card.is_ace?(card)
             |> to(be_false())
    end
  end
  ...
  describe "Card.is_ten?/1" do
    it "returns true" do
      expect Card.is_ten?(ten())
             |> to(be_true())
    end

    it "returns false" do
      expect Card.is_ten?(nine())
             |> to(be_false())
    end
  end
  ...
  describe "Card.one_or_eleven/3" do
    ...
    it "doing a hard count with a total of 10 returns 1" do
      expect Card.one_or_eleven(:hard, 1, 10)
             |> to(eq 1)
    end
  end
end
